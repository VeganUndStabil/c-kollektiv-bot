...
export class DiscordElementFactories {
  ...
  static _applyOption(
    ...
    (builder as any)[`add${upperFirst(option.options.type)}Option`].call(
      builder,
      (builder: any) => {
        applySetters(builder, option.options, replacements);
        if ((option as any).children?.length && "addChoices" in builder)
          builder.addChoices(...(option as any).children.map((it: any) => it.options));
        return builder;
      },
    );
  }
        ...
        case "subCommandGroup":
          cmd.addSubcommandGroup(it => {
            for (const [key, value] of Object.entries(child)) {
              ...
              if (setter in it) (it as any)[setter].call(it, value);
            }
