...
import { PromiseOr } from "src/util/promises";
import {
  ...
  Interaction, ModalActionRowComponent,
...
): CommandInteractionOption<any> | undefined {
...
function Field(name: string) {
  return createInteractionInjector(() => (interaction) => {
    if ("fields" in interaction) return ((interaction.fields as any).getField(name));
  })
}
...
function Option(name: string) {
  return createInteractionInjector(() => (interaction) => {
    if ("options" in interaction) return (interaction.options as any)[name];
  });
}
      ...
      async function ({ context, params: [interaction], meta }) {
        ...
        if (paramMeta.length) {
          const parameters = await Promise.all(
            paramMeta.map((extract: any) => extract(interaction, context)),
          );
          return context[name].call(context, ...parameters);
        }
        const params =
          (await parameters?.(interaction as unknown as T, context, meta)) ?? [];
